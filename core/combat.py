...
class CombatEngine:
    def __init__(self, towers, paredes, unidades, base):
        # recibe listas de objetos: torres, paredes, unidades, y el objeto base
        self.towers = towers
    ...
    def ejecutar_ronda(self):
        # ejecuta turnos hasta que termine el combate y devuelve el resultado
    ...
    def _distancia(self, pos1, pos2):
        # distancia simple entre dos casillas (fila, columna)
        if pos1 is None or pos2 is None:
            return 999
        return abs(pos1[0] - pos2[0]) + abs(pos1[1] - pos2[1])
 
    def _buscar_unidad_en_rango(self, torre):
        # devuelve la primera unidad viva dentro del rango de la torre
        for unidad in self.unidades:
            if unidad.esta_vivo() and self._distancia(torre.position, unidad.position) <= torre.rango:
                return unidad
        return None
 
    def _buscar_obstaculo_en_camino(self, unidad):
        # devuelve la primera torre o pared viva en el camino de la unidad
    ...
    def _esta_vivo(self, objeto):
        # Tower y Unidad usan esta_vivo(), Pared usa esta_viva()
        if hasattr(objeto, "esta_vivo"):
            return objeto.esta_vivo()
        return objeto.esta_viva()
 
    def _fase_torres(self):
        # cada torre viva ataca a la unidad más cercana dentro de su rango
    ...
    def _fase_unidades(self):
        # cada unidad viva avanza, ataca torres/paredes en su camino, o daña la base
        for unidad in self.unidades:
    ...
    def _valor_unidad(self, unidad):
        # dinero que gana el defensor por eliminar esta unidad
    ...
    def _resultado(self, ganador):
        return {
            ...
            "dinero_defensor": self.dinero_defensor_ganado,
            "dinero_atacante": self.dinero_atacante_ganado,
            "base_hp_restante": self.base.hp,
        }